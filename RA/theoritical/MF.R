...
SimulateODE <- function(parameters){
  ...
  parms <- parameters
  pop_size <- ode(func = patch_feedback_model, times = times, y = state, parms = parms, "rk4")
  return(pop_size)
}
...
Data <-
  ...
  select(d, sigma1b, alpha1, alpha2, Result) %>%
  unnest(Result) %>%
  rename("Scenario" = "...1") 
save(Data, file = "/Users/yupeitseng/Documents/RA/Theoritical/MF_data.RData")
load("/Users/yupeitseng/Documents/RA/Theoritical/MF_data.RData")
...
coexsist <- Data %>% 
  ...
  group_by(d, alpha1, alpha2, sigma1b, Scenario, Variable) %>% 
  summarise(Mean_density = mean(Density)) %>% 
  pivot_wider(names_from = Variable, values_from = Mean_density) %>% 
  mutate(d = as.factor(d), alpha1 = as.factor(alpha1), alpha2 = as.factor(alpha2), sigma1b= as.factor(sigma1b))
...
#plotting
png(filename="/Users/yupeitseng/Documents/RA/Theoritical/plot/MF_weak.png", width = 1600, height = 1200)
Data %>% 
  filter(sigma1b == 0) %>% 
  gather(key = Variable, value = Density, Ma1:P2) %>% 
  ggplot() +
  geom_line(aes(x = time, 
                ...
                color = Variable))+
  theme_classic()+
  geom_text(x = 5000, y = 0.05, aes(label = result), data = coexsist %>% filter(sigma1b == 0))+
  theme(text = element_text(size = 15))+
  facet_grid(d~alpha1 + alpha2)
dev.off()

png(filename="/Users/yupeitseng/Documents/RA/Theoritical/plot/MF_weak_9500_10000.png", width = 1600, height = 1200)
Data %>% 
  filter(sigma1b == 0) %>% 
  filter(time %in% seq(from = 9500, to = 10000, by = 0.1)) %>%
  gather(key = Variable, value = Density, Ma1:P2) %>% 
  ...
  geom_line(aes(x = time, 
                ...
                color = Variable))+
  theme_classic()+
  geom_text(x = 9750, y = 0.05, aes(label = result), data = coexsist %>% filter(sigma1b == 0))+
  theme(text = element_text(size = 15))+
  facet_grid(d~alpha1 + alpha2)
dev.off()

png(filename="/Users/yupeitseng/Documents/RA/Theoritical/plot/MF_strong.png", width = 1600, height = 1200)
Data %>% 
  filter(sigma1b == 2) %>% 
  gather(key = Variable, value = Density, Ma1:P2) %>% 
  ggplot() +
  geom_line(aes(x = time, 
                ...
                color = Variable))+
  theme_classic()+
  geom_text(x = 5000, y = 0.05, aes(label = result), data = coexsist %>% filter(sigma1b == 2))+
  theme(text = element_text(size = 15))+
  facet_grid(d~alpha1 + alpha2)
dev.off()

png(filename="/Users/yupeitseng/Documents/RA/Theoritical/plot/MF_strong_9500_10000.png", width = 1600, height = 1200)
Data %>% 
  filter(sigma1b == 2) %>% 
  filter(time %in% seq(from = 9500, to = 10000, by = 0.1)) %>%
  gather(key = Variable, value = Density, Ma1:P2) %>% 
  ...
  geom_line(aes(x = time, 
                ...
                color = Variable))+
  theme_classic()+
  geom_text(x = 9750, y = 0.05, aes(label = result), data = coexsist %>% filter(sigma1b == 2))+
  theme(text = element_text(size = 15))+
  facet_grid(d~alpha1 + alpha2)
dev.off()
...
#check the data
View(Data %>% 
  filter(time %in% seq(from = , to = 5000, by = 0.02)) %>% 
  filter(sigma1b == 0 & d == 0.3 & alpha1 == 0 & alpha2 == 0))
