#Differential soil conditioning----
#### Load library
library("deSolve")
library("tidyverse")
...
Data <-
  ...
  select(-competition) %>% 
  mutate(Par = pmap(., c)) %>%
  mutate(Scenario = paste("dispersal = ", d, ", competition = ", paste(alpha1, alpha2, sep = ","), ", m_effect = ", gb2)) %>%
  mutate(Sim = map(Par, SimulateODE)) %>%
  mutate(Result = map2(Scenario, Sim, bind_cols)) %>%
  select(d, gb2, alpha1, alpha2, Result) %>%
  unnest(Result) %>%
  rename("Scenario" = "...1") 
save(Data, file = "/Users/yupeitseng/Documents/RA/Theoritical/SC_data.RData")
load("/Users/yupeitseng/Documents/RA/Theoritical/SC_data.RData")
...
#Coexsistence
coexsist <- Data %>% 
  filter(time %in% seq(from = 99900, to = 99999, by = 0.1)) %>% 
  gather(key = Variable, value = Density, Ma1:P2) %>% 
  group_by(d, alpha1, alpha2, gb2, Scenario, Variable) %>% 
  summarise(Mean_density = mean(Density)) %>% 
  pivot_wider(names_from = Variable, values_from = Mean_density) %>% 
  mutate(d = as.factor(d), alpha1 = as.factor(alpha1), alpha2 = as.factor(alpha2), gb2= as.factor(gb2))
...
#plotting
png(filename="/Users/yupeitseng/Documents/RA/Theoritical/plot/SC_strong.png", width = 1600, height = 1200)
Data %>% 
  filter(gb2 == 0.3) %>% 
  gather(key = Variable, value = Density, Ma1:P2) %>% 
  ggplot() +
  geom_line(aes(x = time, 
                ...
                color = Variable))+
  theme_classic()+
  geom_text(x = 50000, y = 3, aes(label = result), data = coexsist %>% filter(gb2 == 0.3))+
