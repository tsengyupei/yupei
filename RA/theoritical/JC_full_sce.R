...
Data <-
  crossing(
    d = c(0, 2, 5, 8, 10, 13, 18, 20),
    ga1 = 1.6,
    ga2 = 1, 
    gb1 = 1,
    gb2 = 2, 
    alpha1 = c(1, 0.2, 0.6, 1.1, 1.5, 1.8, 2.1, 2.4, 0.6, 0.6, 1.8, 1.8, 0),
    alpha2 = c(1, 0.5, 0.9, 1.4, 1.8, 2.1, 2.4, 2.7, 0.7, 0.8, 1.9, 2.0, 0),
    phi1 = 0.1, 
    phi2 = 0.1, 
    c11 = 1, 
    c22 = 0.8, 
  ...
  filter(competition == "1,1" | competition == "0.2,0.5" | competition == "0.6,0.9" | competition == "1.1,1.4"| competition == "1.5,1.8"| competition == "0,0"| competition == "2.1,2.4"| competition == "2.4,2.7"| competition == "0.6,0.7"| competition == "0.6,0.8"| competition == "1.8,1.9"| competition == "1.8,2"| competition == "1.8,2.1") %>%
  select(-competition) %>% 
  mutate(Par = pmap(., c)) %>%
  ...
  unnest(Result) %>%
  rename("Scenario" = "...1")
save(Data, file = "/Users/yupeitseng/Documents/RA/Theoritical/JC_full_sce_data.RData")
load("/Users/yupeitseng/Documents/RA/Theoritical/JC_full_sce_data.RData")
#Coexsistence
coexsist <- Data %>% 
...
#### Plot data
#all
png(filename="/Users/yupeitseng/Documents/RA/Theoritical/plot/JC_strong_full_all.png", width = 1600, height = 1200)
Data %>% 
  filter(sigma1a ==  -6)%>% 
  gather(key = Variable, value = Density, Ma1:P2) %>% 
...
#1500-2000 strong
png(filename="/Users/yupeitseng/Documents/RA/Theoritical/plot/JC_strong_full_1500_2000.png", width = 1600, height = 1200)
Data %>% 
  filter(sigma1a ==  -6)%>% 
  gather(key = Variable, value = Density, Ma1:P2) %>% 
  # filter(time %in% seq(from = 0, to = 10, by = 0.02)) %>%
...
Data %>% filter(alpha1 == 1.1 & alpha2 == 1.4) %>% filter(sigma1a == -6) %>% filter(d == 18 | d == 20) %>% filter(time %in% seq(from = 1500, to = 2000, by = 0.1)) %>% group_by(d) %>% summarise_at(vars(Ma1:P2), min)

png(filename="Users/yupeitseng/Documents/RA/Theoritical/plot/JC_weak_full_1500_2000.png", width = 1600, height = 1200)
Data %>% 
  filter(sigma1a  == -0.32)%>% 
  gather(key = Variable, value = Density, Ma1:P2) %>% 
  # filter(time %in% seq(from = 0, to = 10, by = 0.02)) %>%
